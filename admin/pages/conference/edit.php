...
$district = getOne($uri[4], "conference");
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    if ($lang_count >= 3) {
                        ...
                        <input type="text" class="form-control mb-3" name="yonalishEn" value="<?=$district['yonalish_uz']; ?>">
                        <div class="row mb-3">
...
if (isset($_POST['save'])) {
    ...
    if ($sizeFile >= 10240) {
       ...
       if ($_FILES['photo']['size'] == 0) {
            $sqlblog = "update conference set name_uz = '{$nameuz}',name_ru = '{$nameru}',name_en = '{$nameen}',content_uz= '{$contentuz}',content_ru = '{$contentru}',content_en = '{$contenten}', soni='{$son}', kuni='{$kun}', yonalish_uz='{$yonalishuz}', yonalish_ru='{$yonalishru}', yonalish_en='{$yonalishen}' where id = {$district['id']}";
            ...
            if (strcmp($stringEquals, "image") !== 0) {
                ...
                $sqlblog = "update blog set name_uz = '{$nameuz}',name_ru = '{$nameru}',name_en = '{$nameen}',content_uz= '{$contentuz}',content_ru = '{$contentru}',content_en = '{$contenten}',photo = '{$data}', , soni='{$son}', kuni='{$kun}', yonalish_uz='{$yonalishuz}', yonalish_ru='{$yonalishru}', yonalish_en='{$yonalishen}' where id = {$district['id']}";
                $rblog = mysqli_query($con, $sqlblog);
